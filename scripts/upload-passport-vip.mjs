// One-time script: upload PassportVIPFinal.png from ~/Downloads to R2
// Usage: cd Luna-Fantasy-Main && node scripts/upload-passport-vip.mjs
...
// Load .env.local manually (same pattern as upload-passport-assets.mjs)
...
const UPLOADS = [
  {
    key: 'butler/backgrounds/PassportVIPFinal.png',
    file: `${DOWNLOADS}/PassportVIPFinal.png`,
...
// Purge Cloudflare CDN cache so the new file is served immediately
if (CLOUDFLARE_ZONE_ID && CLOUDFLARE_API_TOKEN && uploadedUrls.length > 0) {
  try {
    ...
    if (res.ok) {
      console.log('\nCDN cache purged for uploaded URL');
    } else {
