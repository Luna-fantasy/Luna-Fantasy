...
import { listPropertyOwners, grantPropertyToUser } from '@/lib/admin/valecroft';
...
export async function GET(_req: NextRequest, ctx: Ctx) {
    ...
    try {
        const owners = await listPropertyOwners(ctx.params.key);
        ...
        console.error('[property owners GET]', err);
...
export async function POST(req: NextRequest, ctx: Ctx) {
    ...
    const result = await grantPropertyToUser(discordId, ctx.params.key);
    if (!result.ok) {
        const messages: Record<string, string> = {
            unknown_property: 'No property with that key.',
            inactive: 'Property is inactive — toggle it active before granting.',
            already_owns_this: 'User already owns this property.',
            already_owns_other: 'User already owns a different property. Revoke that one first.',
        };
        return NextResponse.json({ error: messages[result.reason ?? ''] ?? 'Grant failed', reason: result.reason }, { status: 400 });
    }

    await logAdminAction({
        adminDiscordId: adminId,
        adminUsername: adminName,
        action: 'valecroft_grant_property',
        before: null,
        after: { discordId, propertyKey: ctx.params.key },
        metadata: { discordId, propertyKey: ctx.params.key },
        ip: getClientIp(req),
    });
