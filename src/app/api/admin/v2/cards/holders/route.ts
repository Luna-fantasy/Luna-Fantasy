...
export async function GET(req: NextRequest) {
  ...
  const rarity = (sp.get('rarity') ?? '').trim().toUpperCase().slice(0, 20);
  ...
  try {
    const pipeline: any[] = [
      { $project: {
        _id: 1,
        items: { $ifNull: ['$items', '$cards'] },
      } },
      { $unwind: { path: '$items', preserveNullAndEmptyArrays: false } },
      { $match: {
        'items.name': name,
        ...(rarity ? { $expr: { $eq: [{ $toUpper: '$items.rarity' }, rarity] } } : {}),
      } },
    ...
    const rows = await db.collection('cards').aggregate(pipeline).toArray();
    ...
    console.error('Cards holders error:', err);
