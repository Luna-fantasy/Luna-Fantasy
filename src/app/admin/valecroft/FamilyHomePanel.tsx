...
import type { ValecroftLore, ValecroftFamilyMember } from '@/lib/admin/valecroft-lore-types';
import { EMPTY_LORE } from '@/lib/admin/valecroft-lore-types';
...
export default function FamilyHomePanel() {
  ...
  const [saved, setSaved] = useState<ValecroftLore>(EMPTY_LORE);
  const [lore, setLore] = useState<ValecroftLore>(EMPTY_LORE);

  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/admin/valecroft/lore', { cache: 'no-store' });
  ...
  const save = () => {
    if (!dirty) return;
    pending.queue({
      label: 'Save Valecroft family & home',
      ...
      run: async () => {
        try {
          ...
          const res = await fetch('/api/admin/valecroft/lore', {
            method: 'PUT',
          ...
          toast.show({ tone: 'success', title: 'Saved', message: 'Valecroft lore updated.' });
        } catch (e) {
  ...
  const updateHome = <K extends keyof ValecroftLore['home']>(patch: Partial<ValecroftLore['home']>) => {
  ...
  const addFamilyMember = () => {
    ...
    const member: ValecroftFamilyMember = {
      id: `${baseId}-${i}`,
      name: { en: '', ar: '' },
      role: { en: '', ar: '' },
      bio: { en: '', ar: '' },
  ...
  const updateMember = (idx: number, patch: (m: ValecroftFamilyMember) => ValecroftFamilyMember) => {
    setLore((l) => ({
      ...l,
  ...
  if (loading) return <div className="av-flows-empty">Loading Valecroft lore…</div>;
  ...
  return (
    <div className="av-vaellore">
      <header className="av-vaellore-head">
        <div>
          <h3>Family &amp; Home</h3>
          <p>Narrative content shown on the public Valecroft page. Stored in <code>bot_config.valecroft_lore</code>. Images upload to R2 under <code>valecroft/</code>.</p>
        </div>
        ...
        <div className="av-banking-investment-grid">
          ...
          <label className="av-banking-field" style={{ gridColumn: '1 / -1' }}>
            ...
            <textarea
              ...
              placeholder="The silent hold of the Valecroft bloodline…"
            />
          </label>
          <label className="av-banking-field" style={{ gridColumn: '1 / -1' }}>
              ...
              <article key={i} className="av-chars-card">
                <div className="av-chars-card-head">
                  ...
                  <div className="av-chars-meta">
                    ...
                    <div className="av-chars-faction">Valecroft</div>
                  </div>
                </div>
                ...
                <div className="av-chars-fields">
                  ...
                  <label className="av-banking-field">
                    ...
                    <input
                      ...
                      maxLength={120}
                      placeholder="Butler of Valecroft"
                    />
                  </label>
                  <label className="av-banking-field">
                    <span>Name (EN)</span>
